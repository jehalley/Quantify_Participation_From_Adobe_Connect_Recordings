...
def get_index_stream(recording_folder_path):
    '''
    indexstram.xml is a file in the adobe connect recordings folder that
...
def get_camera_contributions(index_stream,ftstage,student_ids):
    ...
    for k in student_pause_start_times.keys():
        for start_time in range(len(student_pause_start_times[k])):
            ...
            clean_stops.append(next((x for x in combined_pause_stop_times[k] if x >student_pause_start_times[k][start_time]),student_pause_start_times[k][start_time] ))
